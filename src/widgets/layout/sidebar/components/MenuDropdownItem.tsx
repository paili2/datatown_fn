...
const MenuDropdownItem: React.FC<MenuDropdownItemProps> = ({ name, path, isActive, isOngoing = false, isPro = false }) => {
  ...
  return (
    <li>
      ...
      >
        {name} 123123
        {/* <div className="flex items-center gap-1 ml-auto">{badges.map((b) => (b.flag ? <SidebarFlagBadge key={b.label} label={b.label} active={active} /> : null))}</div> */}
      </Link>
    </li>
  );
};
